...
bio <- read.table("entrega_1/data/measure_bi_2024.csv", header = TRUE, sep = ";",
          stringsAsFactors = FALSE, encoding = "utf-8")
lugares <- read.table("entrega_1/data/samplePoints_2024.csv", 
...
df <- data.frame(code = dat_last$Code, date = dat_last$date, nivel = dat_last$Level, 
                  val = dat_last$Value, name = dat_last$Sample.Point)
...
barplot(df_filtered$val, names.arg = df_filtered$name, main = "Concentración clorofila A en el fondo", 
        xlab = "g/l", cex.names = 0.5, las = 1, space = 0.5, horiz = TRUE)
...
df_ordered <- df_filtered %>% group_by(name) %>% 
...
bio <- read.table("entrega_1/data/measure_bi_2024.csv", header = TRUE, sep = ";",
          stringsAsFactors = FALSE, encoding = "utf-8")
lugares <- read.table("entrega_1/data/samplePoints_2024.csv", header = TRUE, sep = ";", 
          encoding = "utf-8")

names(bio)[names(bio) == 'Sample.Point.Code'] <- 'Code'
names(bio)[names(bio) == 'Date'] <- 'date'
bio$date <- as.Date(bio$date, format = "%d/%m/%Y")
bio$Level <- ifelse(bio$Level == "S", 
        "Superficie", 
        ifelse(bio$Level == "F", 
            "Fondo", 
            bio$Level))
...
# Reducimos el tamaño de los nombres eliminando los paréntesis: <nombre_principal>(<explicacion>)
# Sin embargo, hay que mantener aquellos en los que existan litorales con distinto id y mismo nombre principal
df <- df %>% mutate(nombre_principal = gsub("\\s*\\(.*\\)$|\\s*\\(.*\\)\\s*$|\\s*$", "", nombre), 
                    explicacion = gsub(".*\\((.*)\\)", "\\1", nombre))
df$explicacion[!grepl("\\(", df$nombre)] <- NA
df <- df %>% group_by(nombre_principal) %>% mutate(nombre_final = 
                      ifelse(n_distinct(id) > 1 & !is.na(explicacion), 
                          paste0(nombre_principal, " (", explicacion, ")"), 
                          nombre_principal)) %>% 
                      ungroup()
...
df <- df %>% group_by(id) %>% mutate(numMuestras = n()) %>% 
...
spain <- ne_countries(country = "Spain", scale = "large", returnclass = "sf")
france <- ne_countries(country = "France", scale = "large", returnclass = "sf")
provinces <- ne_states(country = "Spain", returnclass = "sf") %>% filter(region_cod == "ES.PV")
...
ggplot() + 
    ...
    geom_sf(data = france, fill = "#dddddd", color = "white") +
    geom_sf(data = provinces, fill = NA, color = "black", linetype = "dashed") +

    geom_sf(data = df_sf, aes(size = numMuestras), colour = "black", fill= "#0077ff", shape = 21, alpha = 0.7, stroke = 1.0) +
    scale_size(range = c(2,12)) +

    geom_text_repel(data = df_sf, aes(label = nombre_final, geometry = geometry), stat = "sf_coordinates", size = 5.5, force = 15, min.segment.length = 0.3, nudge_x = c(-0.3, 0.3), nudge_y = c(0.15, -0.15), box.padding = 0.6, point.padding = 0.5,) + 
    labs(x = "Longitud", y = "Latitud", size = "Número de Muestras", title = "Mapa del Litoral de Euskadi con Círculos según el Número de Muestras") + 
